...
There are **30,463** rows and **15** columns. I found this by printing the first 10 rows of the data set with this code:
...
The same code from #2 told me the data types for each variable:
*record_id <dbl>, month <dbl>, day <dbl>, year <dbl>, plot_id <dbl>, species_id <chr>, sex <chr>, hindfoot_length <dbl>, weight <dbl>, date <date>, day_of_week <chr>, plot_type <chr>, genus <chr>, species <chr>, taxa <chr>*
***<dbl>* meaning "double" for numeric, and *<chr>* meaning character**
...
geom_point(alpha = .15) + 
  ...
  labs(x = "Weight (grams)",
       ...
       title = "Weight & Hindfoot Length in These Animal Species")
```
...
```{r}
#| label: boxplot-with-jitter-color
...
species_mod <- aov(formula = weight ~ species, 
                   data = surveys)
...
Based on the ANOVA F-test there is significant evidence that at least one mean is significantly different from the rest, meaning that species has an effect on mean weight of an animal. We can draw this conclusion based upon the large F-value of 19,916 and an extremely low p-value of <2e-16 based on the standard significance level of .05.
